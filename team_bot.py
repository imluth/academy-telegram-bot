...
class RedisConnection:
    ...
    @backoff.on_exception(
        backoff.expo, (aioredis.ConnectionError, aioredis.TimeoutError), max_tries=5
    )
...
class RateLimiter:
    ...
    @backoff.on_exception(
        backoff.expo, (aioredis.ConnectionError, aioredis.TimeoutError), max_tries=3
    )
    async def acquire(
        self, user_id: int, action_type: str = "default"
    ) -> Tuple[bool, float]:
        ...
        try:
            ...
            requests = await self.redis.zrangebyscore(
                key, now - self.per_seconds, now, withscores=True
            )
...
class PlaySession:
    ...
    async def set_state(self, state: dict):
        try:
            await self.redis.set(
                f"{self.key_prefix}:state", json.dumps(state), ex=86400
            )
        except Exception as e:
    ...
    async def set_players(self, players: List[Player]):
        try:
            ...
            await self.redis.set(f"{self.key_prefix}:players", players_data, ex=86400)
        except Exception as e:
...
def setup_logging(self):
        ...
        try:
            ...
            for sig in (signal.SIGTERM, signal.SIGINT):
                loop.add_signal_handler(
                    sig, lambda: asyncio.create_task(self.shutdown(app, stop_event))
                )
    ...
    def format_player_list(self, players: List[Player], play_day: str) -> str:
        ...
        list_lines = [f"*{day} Play {time}*", f"{location}\n", "*Players List:*"]
    ...
    async def handle_play_response(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        ...
        try:
            ...
            existing = next(
                (
                    p
                    for p in players
                    if p.user_id == user.id and p.is_plus_one == is_plus_one
                ),
                None,
            )
            if existing:
    ...
    async def _handle_leave(
        self, session: PlaySession, players: List[Player], user, query: CallbackQuery
    ) -> bool:
    ...
    async def _handle_full_list(
        ...
        try:
            ...
            await session.set_state(
                {**state, "teams": [[p.to_dict() for p in team] for team in teams]}
            )

            teams_message = self.format_teams_message(teams, state.get("play_day"))
    ...
    def _create_balanced_teams(self, players: List[Player]) -> List[List[Player]]:
        ...
        try:
            ...
            regular_players.sort(key=lambda p: (-p.rating, p.join_time or datetime.max))
            ...
            team_white.extend(
                plus_one_players[
                    remaining_slots_black : remaining_slots_black
                    + remaining_slots_white
                ]
            )
    ...
    async def _update_play_message(
        self, bot, chat_id: int, message_id: int, players: List[Player], play_day: str
    ):
        ...
        try:
            ...
            await asyncio.sleep(e.retry_after)
            await self._update_play_message(bot, chat_id, message_id, players, play_day)
        except Exception as e:
